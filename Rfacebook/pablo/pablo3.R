...
mat <- getNetwork(tokenX, format="adj.matrix")
...
labels <- c("Rodzina", "Liceum", "Uniwersytet", "Politechnika")#, "Caixa Fellow", "NYC", "Others")
pq <- pq + scale_color_discrete(labels=labels)
pq

## we can also add the labels to the plot
d$label <- NA
d$label[26] <- "Mateusz £uczko"
pq <- pq + geom_text(aes(x=x, y=y, label=label), data=d, color="red", size=3)
...
pq <- p + geom_segment(
   ...
   geom_point(color="grey20", aes(fill=cluster), shape=21, size=3) +
